...
export function PracticeSearchDialog({
  open,
  onOpenChange,
  jumpInput,
  setJumpInput,
        ...
        <div className="space-y-3 overflow-auto pr-1 min-h-0">
          ...
          {jumpInput.trim() ? (
            <div className="text-xs text-muted-foreground">
              {jumpLoading ? "搜索中..." : `匹配 ${computedMatches.length} 条`}
            </div>
          ) : null}
                ...
                {computedMatches.slice(0, 10).map((m) => (
                  ...
                  >
                    <div className="flex items-center gap-2 text-sm">
                      <span className="inline-flex items-center px-2 py-0.5 rounded border text-xs bg-gray-50">
                        {m.j}
                      </span>
