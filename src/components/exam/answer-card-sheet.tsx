...
export function AnswerCardSheet({ open, onOpenChange, questions, answers, flags, finished, filter, onChangeFilter, onJumpTo }: AnswerCardSheetProps) {
      ...
      <SheetContent side="right">
        ...
        <div className="px-4 space-y-3">
          <div className="flex items-center justify-between gap-3 text-sm">
            <div className="text-muted-foreground">已答 {answeredCount} / {questions.length}｜标记 {flaggedCount}</div>
            <div className="flex items-center gap-2">
              <Button size="sm" variant={filter === "all" ? "default" : "outline"} onClick={() => onChangeFilter("all")}>全部</Button>
              <Button size="sm" variant={filter === "unanswered" ? "default" : "outline"} onClick={() => onChangeFilter("unanswered")}>未答</Button>
              <Button size="sm" variant={filter === "flagged" ? "default" : "outline"} onClick={() => onChangeFilter("flagged")}>标记</Button>
            </div>
          </div>
          <Separator />
          <div className="flex items-center justify-between">
            <div className="text-sm text-muted-foreground">点击题号跳转</div>
            <Button size="sm" variant="outline" onClick={() => { const i = firstUnansweredIndex(); if (i >= 0) { onJumpTo(i); onOpenChange(false); } }}>跳到首个未答</Button>
          </div>
          <div className="grid grid-cols-6 gap-2 sm:grid-cols-8">
            {questions.map((q, i) => {
              ...
              const selectedForFilter = filter === "all" || (filter === "unanswered" && !isAnswered) || (filter === "flagged" && isFlagged);
              ...
              return (
                <button
                  key={`q-${i}`}
                  className={"relative h-9 rounded-md border text-sm font-medium transition-colors " + (isAnswered ? "bg-primary text-primary-foreground" : "bg-muted text-foreground")}
                  onClick={() => { onJumpTo(i); onOpenChange(false); }}
                >
                  <span>{i + 1}</span>
